...
def validate_geographic_data(data_text: str | None):
    ...
    data_match = re.search(r'Selected bounds: \(([-\deE.]+), ([-\deE.]+), ([-\deE.]+), ([-\deE.]+)\)', data_text)
    ...
    if data_match:
        ...
        assert min_lon != 0 and min_lat != 0 and max_lon != 0 and max_lat != 0, "Coordinates should not be zero"
        assert min_lon < max_lon and min_lat < max_lat, "Min values should be less than max values"
        assert -180 <= min_lon <= 180 and -180 <= max_lon <= 180, "Longitude should be in range [-180, 180]"
        assert -90 <= min_lat <= 90 and -90 <= max_lat <= 90, "Latitude should be in range [-90, 90]"
...
def test_bbox_interaction_workflow(solara_test, page_session, sample_map):
    ...
    assert interaction_result is not None, "Expected bbox selected_bounds to be available after interaction"
    ...
    print(f"✅ BBox interaction completed with valid coordinates: {bounds_string}")